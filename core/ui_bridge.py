...
When Celery workers execute scan tasks, all ui_worker_* calls happen in the
worker process — isolated from the watchdog that owns the Rich Live render loop.
This module closes that gap:
  ...
  Watchdog process← UIEventSubscriber._loop()  ← Redis channel hunt3r:ui_events
                                                   ↓
                                          local ui_worker_* calls
                                                   ↓
                                          _workers dict updated
                                                   ↓
...
class UIEventPublisher:
    ...
    def publish(self, event_type: str, **kwargs) -> None:
        if not self._ok:
            return
        try:
            payload = json.dumps({"type": event_type, **kwargs}, default=str)
...
class UIEventSubscriber:
    """Subscribe to UI events and replay them as local ui_worker_* calls.
...
def _dispatch_event(event: dict) -> None:
    """Translate an incoming PubSub event dict into a local ui_worker_* call."""
